...
1.本项目已经部署到线上，地址是http://47.104.239.130
2.使用移动端登录时就会默认跳转到web移动端首页，使用pc端登录就会默认跳转到管理后台
项目使用的技术框架：
3.后台支持框架:Shiro + Spring + SpringMvc + Mybatis + Redis + nginx
4.pc端页面支持:FreeMarker+bootstrap
5.移动端web页面支持:(待补充)
6.本项目采用前后端分离架构模式，用nginx根据访问的路径后以及访问设备类型进行请求转发。
