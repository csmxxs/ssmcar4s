		...
		<script >
			so.init(function(){
				...
				so.id('deleteAll').on('click',function(){
					...
					if(!checkeds.length){
						return layer.msg('请选择要清除的角色。',so.mdefault),!0;
					}
					var array = [];
			...
			function selectPermissionById(id){
				...
				$.post("${basePath}/permission/selectPermissionById.shtml",{id:id},function(result){
					...
					if(result && result.length){
						...
						return layer.msg('没有获取到权限数据，请先添加权限数据。',so.mdefault);
					}
				},'json');
			}
			</@shiro.hasPermission>
						...
						<#if page?exists && page.list?size gt 0 >
							<#list page.list as it>
								<tr>
									...
									<td>${it.name}</td>
									<td>${it.type}</td>
									<td permissionIds="${it.permissionIds?default('')}">${it.permissionNames?default('无操作权限')}</td>
									<td>
										<@shiro.hasPermission name="/permission/addPermission2Role.shtml">
											<i class="glyphicon glyphicon-share-alt"></i><a href="javascript:selectPermissionById(${it.id});">修改权限</a>
										</@shiro.hasPermission>
									</td>
								</tr>
							</#list>
