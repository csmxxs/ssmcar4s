...
import com.ylms.core.mybatis.page.MysqlDialect;
...
public class BaseMybatisDao<T> extends SqlSessionDaoSupport {
	...
	public BaseMybatisDao() {
		try {
			//返回表示当前Class 所表示的实体（类、接口、基本类型或 void）的直接超类的Type。
	...
	public Pagination findByPageBySqlId(String sqlId,
		...
		try {
			...
			PreparedStatement ps = getPreparedStatement(countCode,
					countSql.getParameterMappings(), params, conn);
			ps.execute();
	...
	public Pagination findPage(String sqlId, String countId,
		...
		String countCode = "";
		BoundSql countSql = null;
		if (StringUtils.isBlank(countId)) {// 如果该参数不存在
			countCode = sqlcode;
			countSql = boundSql;
		} else {
		...
		try {
			 ...
			 * 处理Count
			 */
			PreparedStatement ps = getPreparedStatement4Count(countCode,
					countSql.getParameterMappings(), params, conn);
			ps.execute();
			ResultSet set = ps.getResultSet();
	...
	public Pagination findPage(Map<String, Object> params, Integer pageNo,
		...
		return findPage(DEFAULT_SQL_ID, DEFAULT_COUNT_SQL_ID, params, pageNo,
				pageSize);
	}
	 ...
	 * */
	@SuppressWarnings("rawtypes")
	public Pagination findPage(String sqlId,Map<String, Object> params, Integer pageNo,
			Integer pageSize) {
		String findTaskCount="findTaskCount";//查询总记录数
		return findPage(sqlId,findTaskCount, params, pageNo,
				pageSize);
	}
	...
	private PreparedStatement getPreparedStatement(String sql,
		...
		for (int i = 0; i < parameterMappingList.size(); i++) {
			ps.setObject(index++,
					params.get(parameterMappingList.get(i).getProperty()));
		}
		return ps;
	}
