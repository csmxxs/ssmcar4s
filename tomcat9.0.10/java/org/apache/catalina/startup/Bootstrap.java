...
/**
 * 
 * 用于卡塔利纳的引导加载程序。此应用程序构造了一个类加载器，用于加载Calalina内部类（通过累积在Casalina。
 * HOST中的“服务器”目录中找到的所有JAR文件），并启动容器的定期执行。这种迂回方法的目的是将Calalina内部类
 * （以及它们依赖的任何其他类，如XML解析器）保持在系统类路径之外，因此对应用程序级别类不可见。 
